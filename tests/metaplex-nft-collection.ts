...
import { MetaplexNftCollection } from "../target/types/metaplex_nft_collection";
import { PublicKey, Keypair, SystemProgram } from "@solana/web3.js";
import { assert } from "chai";
import { MPL_CORE_PROGRAM_ID } from "@metaplex-foundation/mpl-core";

describe("metaplex-nft-collection", () => {
  // Configure the client to use the local cluster.
  const provider = anchor.AnchorProvider.env();
  anchor.setProvider(provider);
  const program = anchor.workspace.MetaplexNftCollection as Program<MetaplexNftCollection>;
  ...
  describe("ThawNft", () => {
    it("Thaw an NFT (minted frozen by default)", async () => {
      await program.methods
        .thawNft()
        .accountsStrict({
  ...
  describe("FreezeNft", () => {
    it("Freeze an NFT (after thawing)", async () => {
      await program.methods
        .freezeNft()
        .accountsStrict({
          authority: creator.publicKey,
